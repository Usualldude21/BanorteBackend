...
export function createUiSystemPrompt(experienceScope: FinancialExperienceScope = "full"): string {
  ...
  return [
    ...
    ...(personalBankingOnly ? [
      ...
      "Para cuentas y saldos genera una composición breve: una o dos métricas y una tabla de saldos; nunca un dashboard ni una tabla sin contexto.",
      "Para explicar un cambio usa una conclusión, máximo cuatro métricas y sólo las dos categorías con mayor variación. No generes una tabla completa de categorías sin que el usuario la pida.",
      "Para gasto por categoría muestra una visualización breve y una tabla de detalle; para movimientos muestra una tabla navegable con fecha, descripción, categoría e importe.",
      "Prioriza una gráfica comparativa breve sobre un dashboard. El detalle de movimientos se muestra únicamente cuando el usuario lo solicita.",
    ] : []),
    ...
    "Cuando exista una fuente derived_liquidity_analysis, usa un heatmap enlazado a timeline para el patrón por mes y día, monthlyLows para los mínimos y precedingExpenses para los gastos previos; no reconstruyas esa relación desde fuentes crudas.",
